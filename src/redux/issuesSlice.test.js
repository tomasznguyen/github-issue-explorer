...
describe("issuesSlice", () => {
  ...
  describe("reducers/fetchIssues", () => {
    ...
    it("sets items and sets status to succeeded when fetchIssues is fulfilled", () => {
      ...
      expect(state.data[page]).toHaveLength(2);
    });
    ...
    it("clears the error message on successful load", () => {
      initialState = {
        error: "Not Found",
        data: {},
        issuesPerPage: 10,
  ...
  describe("reducers/fetchRepositoryInformation", () => {
    it("sets status to fetching when fetchRepositoryInformation is pending", () => {
      const action = { type: fetchRepositoryInformation.pending.type };
      const state = reducer(initialState, action);

      expect(state.status).toEqual(FetchingStatus.Fetching);
    });

    it("sets items and sets status to succeeded when fetchRepositoryInformation is fulfilled", () => {
      const page = 1;
      const action = {
        type: fetchRepositoryInformation.fulfilled.type,
        payload: {
          issues,
          issuesPerPage: 20,
      ...
      expect(state.status).toEqual(FetchingStatus.Succeeded);
      expect(state.data[page]).toHaveLength(2);
      expect(state.issuesPerPage).toBe(20);
      expect(state.organization).toBe("facebook");
      expect(state.pageCount).toBe(1);
      expect(state.repository).toBe("react");
    });

    it("sets error and sets status to failed when fetchRepositoryInformation is rejected", () => {
      const action = { type: fetchIssues.rejected.type, payload: "Not Found" };
      const state = reducer(initialState, action);
